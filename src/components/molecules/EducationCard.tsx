    ...
    <div className={`timeline__item reveal reveal--left reveal-s${index}`} style={{ position: 'relative' }}>
      ...
      <div className="glass-card glass-card--cyan" style={{ padding: '1.75rem 2.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
          ...
          <div>
            <p style={{ fontSize: '0.78rem', color: 'rgba(var(--fg-rgb), 0.4)', fontWeight: 500 }}>{education.period}</p>
            <p style={{ color: 'var(--cyan-soft)', fontWeight: 600, fontSize: '0.95rem' }}>{education.school}</p>
          </div>
        </div>

        <h3 style={{ fontSize: '1.05rem', fontWeight: 700, color: 'var(--fg)', lineHeight: 1.35, marginBottom: education.specialization ? '0.4rem' : 0 }}>
          {education.degree}
        </h3>
        {education.specialization && (
          <p style={{ fontSize: '0.85rem', color: 'rgba(var(--fg-rgb), 0.4)' }}>{education.specialization}</p>
        )}
      </div>
    </div>
  );
