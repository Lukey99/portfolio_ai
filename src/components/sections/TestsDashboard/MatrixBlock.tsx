...
export function MatrixBlock() {
  ...
  return (
    <div ref={ref} style={{ overflowX: 'auto' }}>
      <table
        style={{ borderCollapse: 'separate', borderSpacing: 0, width: '100%', minWidth: '480px' }}
      >
        <thead>
          <tr>
            <th style={{ width: '120px', paddingBottom: '0.75rem', textAlign: 'left' }}>
              ...
              >
                {t.testsDashboard.componentColLabel}
              </span>
            </th>
            {TEST_COLS.map(col => {
                ...
                <th key={col} style={{ paddingBottom: '0.75rem', textAlign: 'center' }}>
                  <button
                    ...
                    style={{
                      fontSize: '0.62rem',
                      fontWeight: 700,
                      color: isActive ? COL_COLORS[col] : 'rgba(var(--fg-rgb), 0.45)',
                      background: isActive ? mix(COL_COLORS[col], 9) : 'transparent',
                      border: `1px solid ${isActive ? mix(COL_COLORS[col], 31) : 'rgba(var(--overlay-rgb), 0.12)'}`,
                      borderRadius: '999px',
            ...
            <th style={{ paddingBottom: '0.75rem', textAlign: 'right' }}>
              <span
                style={{ fontSize: '0.58rem', color: 'rgba(var(--fg-rgb), 0.3)', fontWeight: 600 }}
              >
                {t.testsDashboard.covColLabel}
              </span>
            </th>
          </tr>
        </thead>
          ...
          {COMPONENTS.map((comp, ri) => {
            ...
            return (
              <motion.tr
                ...
                onMouseEnter={() => setHoveredRow(comp.name)}
                onMouseLeave={() => setHoveredRow(null)}
                style={{
                  background: isHovered ? 'rgba(var(--overlay-rgb), 0.04)' : 'transparent',
                  borderRadius: '0.5rem',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                    ...
                    <span
                      style={{
                        ...
                        color: isHovered ? 'var(--fg)' : 'rgba(var(--fg-rgb), 0.6)',
                    ...
                    <td key={col} style={{ textAlign: 'center', padding: '0.35rem 0.25rem' }}>
                      <motion.div
                        ...
                        style={{
                          ...
                          borderRadius: '50%',
                          margin: '0 auto',
                          background: isCovered ? COL_COLORS[col] : 'transparent',
                          border: isCovered ? 'none' : '1.5px solid rgba(var(--overlay-rgb), 0.15)',
                          boxShadow:
                            isCovered && isHovered ? `0 0 8px ${mix(COL_COLORS[col], 50)}` : 'none',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', marginLeft: 'auto' }}>
          <div
            style={{
              ...
              height: '10px',
              borderRadius: '50%',
              background: 'var(--t-unit)',
            }}
          />
          <span style={{ fontSize: '0.6rem', color: 'rgba(var(--fg-rgb), 0.38)' }}>
            {t.testsDashboard.coveredLabel}
          </span>
          <div
            style={{
              width: '10px',
              height: '10px',
              borderRadius: '50%',
              border: '1.5px solid rgba(var(--overlay-rgb), 0.2)',
              marginLeft: '0.5rem',
            }}
          />
          <span style={{ fontSize: '0.6rem', color: 'rgba(var(--fg-rgb), 0.38)' }}>
            {t.testsDashboard.notCoveredLabel}
          </span>
        </div>
      </div>
    </div>
