...
export function CoverageView() {
  ...
  const priorityMetaColors: Record<Priority, string> = {
    high: 'var(--t-perf)',
    medium: 'var(--t-a11y)',
    low: 'var(--t-organism)',
  };
  ...
  return (
    <div ref={ref} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      <div style={CARD}>
        ...
        >
          {t.testsDashboard.coverageLayerLabel}
        </p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {COVERAGE_LAYERS.map((c, i) => (
            <CoverageBar key={c.label} {...c} delay={i * 0.1} />
          ))}
        </div>
      </div>
          ...
          {coverageGaps.map((gap, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  <span style={{ fontSize: 'var(--text-xs)', fontWeight: 700, color: 'var(--fg)' }}>
                    {gap.name}
                  </span>
                  <span
                    style={{
      ...
      <div style={CARD}>
        ...
        >
          {t.testsDashboard.coverageComponentLabel}
        </p>
        <MatrixBlock />
      </div>
    </div>
