...
export function PipelineView() {
  ...
  const slowest = PIPELINE_STATIC.reduce((a, b) =>
    (PIPELINE_SECS[a.label] ?? 0) >= (PIPELINE_SECS[b.label] ?? 0) ? a : b
  );
    ...
    <div ref={ref} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.875rem' }}>
        {[
          ...
          {
            label: statsLabels[2],
            value: t.testsDashboard.pipelineGreenBadge,
            color: 'var(--t-organism)',
          },
        ].map(({ label, value, color }) => (
          <div key={label} style={{ ...CARD, textAlign: 'center', padding: '1rem' }}>
            <div style={{ fontSize: '1.4rem', fontWeight: 900, color, letterSpacing: '-0.03em' }}>
              {value}
            </div>
            <div
              style={{
                  ...
                  <AnimatePresence>
                    {isSelected && (
                      ...
                      >
                        <div
                          style={{
                            background: mix(stage.color, 7),
                            border: `1px solid ${mix(stage.color, 25)}`,
                            borderRadius: '0.6rem',
                            padding: '0.65rem 0.85rem',
                            margin: '0.2rem 0',
                            display: 'flex',
                ...
                return (
                  ...
                  >
                    <motion.button
                      ...
                      style={{
                        flex: 1,
                        background: isSelected ? mix(stage.color, 9) : 'var(--card-bg)',
                        border: `1px solid ${isSelected ? mix(stage.color, 38) : 'rgba(var(--overlay-rgb), 0.09)'}`,
                    ...
                    >
                      ...
                      >
                        ...
                        />
                        <span style={{ fontSize: '0.6rem', color: stage.color, fontWeight: 700 }}>
                          ✓
                        </span>
                      </div>
                      <div
                      ...
                      >
                        {stage.duration}
                        {stage.count !== null && (
                          <span style={{ marginLeft: '0.3rem', color: stage.color }}>
                            ·{stage.count}
                          </span>
                        )}
                      </div>
                      ...
                      >
                        <motion.div
                          ...
                          style={{
                            height: '1px',
                            width: '100%',
                            background: `linear-gradient(90deg, ${mix(pipeline[i].color, 38)}, ${mix(pipeline[i + 1].color, 38)})`,
            ...
            <AnimatePresence>
              {selected !== null && (
                ...
                >
                  <div
                    style={{
                      background: mix(pipeline[selected].color, 7),
                      border: `1px solid ${mix(pipeline[selected].color, 25)}`,
                      borderRadius: '0.6rem',
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: '6px',
                        borderRadius: '50%',
                        background: pipeline[selected].color,
                        flexShrink: 0,
                      }}
                    />
                    <span
                      style={{ fontSize: 'var(--text-xs)', color: 'rgba(var(--fg-rgb), 0.75)' }}
                    >
                      {pipeline[selected].detail}
                    </span>
                  </div>
                </motion.div>
              )}
      ...
      <div style={CARD}>
                  ...
                  >
                    <motion.div
                      ...
                      style={{
                        ...
                        background: stage.color,
                        borderRadius: '999px',
                        boxShadow: isSlowest ? `0 0 8px ${mix(stage.color, 50)}` : 'none',
                      }}
                    />
                  </div>
                  <span
                    style={{
                      fontSize: '0.62rem',
                      fontFamily: 'monospace',
                      color: isSlowest ? stage.color : 'rgba(var(--fg-rgb), 0.45)',
        ...
        <div
          style={{
            ...
            background: mix(slowest.color, 5),
            border: `1px solid ${mix(slowest.color, 19)}`,
            borderRadius: '0.5rem',
            fontSize: '0.62rem',
        ...
        >
          ...
          {t.testsDashboard.pipelineSlowestTemplate} ({slowest.duration} ·{' '}
          {(((PIPELINE_SECS[slowest.label] ?? 0) / TOTAL_SECS) * 100).toFixed(0)}% du total).
        </div>
      </div>
    </div>
  );
