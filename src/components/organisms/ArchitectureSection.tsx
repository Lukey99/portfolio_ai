...
const LAYERS = [
  ...
  },
  {
    ...
    components: ['Header', 'Hero', 'ExperienceSection', 'SkillsSection', 'AIWorkflowSection', 'TestsSection', 'ProjectsSection', 'EducationSection', 'ContactSection'],
  },
  {
    level: 'Templates',
    labelFr: 'Templates',
...
const DEMO_STEPS: { label: string; color: string; component: string; desc: string }[] = [
  ...
  },
  {
    ...
    desc: 'Orchestre la liste d\'expériences et délègue chaque entrée à une molécule.',
  },
  {
    label: 'Molécule',
    color: '#8b5cf6',
    component: 'ExperienceCard',
    desc: 'Assemble Badge + GradientText + textes en carte d\'expérience autonome.',
  },
  {
    label: 'Atomes',
    color: '#22d3ee',
  ...
  return (
    <section ref={ref} style={{ padding: '7rem 1.5rem', backgroundColor: 'var(--bg)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: '64rem', margin: '0 auto', position: 'relative' }}>
        <SectionTitle number="05" label="Architecture" title="Design atomique" />

        <motion.p
          initial={{ opacity: 0, y: 12 }}
          animate={isInView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.5, delay: 0.1 }}
          style={{ fontSize: '1rem', color: 'rgba(var(--fg-rgb), 0.45)', maxWidth: '520px', marginBottom: '3.5rem', lineHeight: 1.7 }}
        >
